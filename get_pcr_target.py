...
#usage: python get_pcr_target.py primers geneseq.fa
#example: python get_pcr_target.py 16s.full.primers.txt eskape.16s.fa > pcr_product.fa
                ...
                for y in rpri.items():
                    ...
                    if len(rma) > 0 :
                        for rst in rma:
                            ...
                            pfpri = x[0]
                            prpri = y[0]
                            print ">%s %s(%s) %s %s(%s) %s %s\n%s" %(name, pfpri,fpri[pfpri],st, reverse_complement.get_rc(prpri),rpri[prpri],st+rst+len(y[0]),len(product), se)
    return 0
